...
// adjust kd values based on time differences
double kd_values[4] = {1.0, 1.0, 1.0, 1.0};  // current kd values for each motor

// Define PID for 4 motors
PID pid1(&input1, &output1, &s1, Kp1, Ki1, kd_values[0], DIRECT);
PID pid2(&input2, &output2, &s2, Kp2, Ki2, kd_values[1], DIRECT);
PID pid3(&input3, &output3, &s3, Kp3, Ki3, kd_values[2], DIRECT);
PID pid4(&input4, &output4, &s4, Kp4, Ki4, kd_values[3], DIRECT);

double e_values[4]={input1,input2,input3,input4};
double time_diff[4]={e_values[0] - e_values[1],e_values[0] - e_values[2],e_values[0] - e_values[3],e_values[0] - e_values[4],};
...
void adjust_kd_for_all(double kd_values[], double time_diff[], int num_motors) {
  for (int i = 0; i < num_motors; i++) {
        kd_values[i] *= abs(time_diff[i]);
    }
  }
//--------------------------------------------------------------------
void setup() {
...
void loop() {
  if((input1!=input1t) || (input2!=input2t) || (input3!=input3t) || (input4!=input4t))
  {
    ...
    Serial.print("K1=");Serial.print(kd_values[0]);Serial.print(", K2=");Serial.print(kd_values[1]);Serial.print(", K3=");Serial.print(kd_values[2]);Serial.print(", K4=");Serial.println(kd_values[3]);
    input1t=input1;
    input2t=input2;
    input3t=input3;
    input4t=input4;
  ...
  input1 = (double)encoder1.read() * 360.0 / 1024.0;
  input2 = (double)encoder2.read() * 360.0 / 1024.0;
  input3 = (double)encoder3.read() * 360.0 / 1024.0;
  input4 = (double)encoder4.read() * 360.0 / 1024.0;
  ...
  e_values[3] = input4;
  //update time_diff
  time_diff[0] = e_values[0] - e_values[1];
  time_diff[1] = e_values[0] - e_values[2];
  time_diff[2] = e_values[0] - e_values[3];
  adjust_kd_for_all(kd_values, time_diff, 4);  
