...
void loop() {
  ...
  float v1 = velocity1/616.0*60.0;
  float v2 = velocity2/616.0*60.0;
  float v3 = velocity3/616.0*60.0;
  float v4 = velocity4/616.0*60.0;
  ...
  float setpoint=100;
  tinhtoan_quangduong_di(setpoint,90);
  calculate_s(setpoint,v1,v2,v3,v4,deltaT1);
...
void tinhtoan_quangduong_di(int v, float degree)//degree integer number pls
{
  ...
  vt1 = v * cos(3.14/4+x)*(-1);
  vt2 = v * sin(3.14/4+x);
  vt3 = v * cos(3.14/4+x);
  vt4 = v * sin(3.14/4+x)*(-1);
} 
