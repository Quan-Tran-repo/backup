...
void go(float d, float angle)
{
  ...
  deltad=dis-distance;
  if (deltad>700) {speed=200;}
  else if (deltad>500) {speed=150 + (deltad-800) * (200-150)/(700-500);}
  else if (deltad>300) {speed=100 + (deltad-400) * (150-100)/(500-300);}
  else if (deltad>50)  {speed=50  + (deltad-100) * (100-50)/(300-50);}
  else if (deltad>0)   {speed=0   + (deltad-0)   * (50-0)/(50-0);}
}
void run_triangle( float v1, float v2, float v3, float v4, float deltaT1){
  
  if (mode == 1) {
      //di vong quanh the gioi
    go(3.606,33.7); //go how many square in what direction?
    calculate_s(speed,v1,v2,v3,v4,deltaT1);
    if(distance > 179) {mode = 2;}
  }
  else if(mode == 2){
    go(6.606,180); //go how many square in what direction?
    calculate_s(speed,v1,v2,v3,v4,deltaT1);
    if(distance > 328) {mode = 3;}
  }
  else if(mode == 3){
    go(8.606,-90); //go how many square in what direction?
    calculate_s(speed,v1,v2,v3,v4,deltaT1);
    if(distance > 427) {mode = 4;}
  }
}
